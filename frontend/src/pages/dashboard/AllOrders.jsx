...
function AllOrders({ authUser }) {
  ...
  const fetchOrders = async ({ queryKey }) => {
    ...
    try {
      const res = await fetch(
        `/api/rumman/v1/admin/get-all-orders?skip=${skip}&search=${search.trim()}`
      );
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["allOrders", page, search],
    queryFn: fetchOrders,
              ...
              <div className="col-span-9">
                <div className="w-full rounded-md">
                  ...
                  <input
                    ...
                    placeholder="By Id, Buyer or Seller Email, Limited Name, Game Username"
                    className="px-3 py-2.5 md:py-2 2xl:py-3  outline-none text-sm 2xl:text-lg rounded-md w-full lg:w-2/4 mt-2 bg-dry"
                  />
                  <OrdersTable
                    authUser={authUser}
                    items={data?.orders}
                    totalCars={data?.totalOrders}
                    page={page}
                    setPage={setPage}
