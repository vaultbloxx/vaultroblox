...
function WithdrawalRequests({ authUser }) {
  ...
  const fetchRequests = async ({ queryKey }) => {
    ...
    try {
      const res = await fetch(
        `/api/rumman/v1/admin/get-withdrawal-requests?skip=${skip}&search=${search.trim()}`
      );
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["withdrawalRequests", page, search],
            ...
            <div className="md:grid grid-cols-12 gap-12 pt-10">
              ...
              <div className="col-span-9">
                <div className="w-full rounded-md">
                  ...
                  <input
                    type="text"
                    placeholder="Search by Id, Seller Email or Username"
